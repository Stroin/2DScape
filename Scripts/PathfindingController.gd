...
func _input(event: InputEvent) -> void:
	...
	and event.pressed:
		...
		var target_cell : Vector2i = clicked_cell      # default target
		var look_at     : Vector2i = Vector2i(-1, -1)  # default “no facing”

		# ----- clicked directly on a tree --------------------------------
		if is_tree:
			look_at     = clicked_cell
			target_cell = _nearest_reachable_neighbour(clicked_cell, start_cell)
			if target_cell == Vector2i(-1, -1):
				return   # tree completely surrounded – nothing to do

		# -----------------------------------------------------------------
		var path : PackedVector2Array = astar_grid.get_point_path(start_cell, target_cell)
		if path.size() > 0:                # allow size == 1
...
func _nearest_reachable_neighbour(tree: Vector2i, start: Vector2i) -> Vector2i:
	...
	var best_len    : int      = 1_000_000       # something large
